...
const viewRoles = async () => {
    ...
    db.query("SELECT employee_role.title, employee_role.id, department.name, employee_role.salary FROM employee_role LEFT JOIN department ON employee_role.department_id = department.id LEFT JOIN employee ON employee_role.id = employee.role_id", function (err, result) {
        if (err) throw err;
        console.log('');
        console.table(result);
    });
...
const viewEmployees = async () => {
    ...
    db.query(`SELECT employee.id, employee.first_name, employee.last_name, employee_role.title, department.name, employee_role.salary, CONCAT(m.first_name, ' ', m.last_name) 'manager_title' FROM employee LEFT JOIN employee_role ON employee.role_id = employee_role.id LEFT JOIN department ON employee_role.department_id = department.id LEFT JOIN employee m ON (employee.manager_id = m.id)`, function (err, result) {
        if (err) throw err;
        console.log('');
        console.table(result);
    });
